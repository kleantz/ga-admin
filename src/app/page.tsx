...
export default function AccountManagement() {
  ...
  const handleCloseDialog = () => {
    ...
    // Small delay to allow animation before clearing selected account
  ...
  const handleEditAccount = (account: Account) => {
    ...
    // Keep the details dialog open for stacking
  }

  const handleCloseEditDialog = () => {
    setIsEditDialogOpen(false)
    setTimeout(() => setAccountToEdit(null), 200)
    // Details dialog remains open
  }
  ...
  const confirmSuspendAccount = () => {
    if (accountToSuspend) {
      ...
      // TODO: API call to suspend/activate account
      // Update the account status in mockData
      toast.success(`Account ${action}`, {
        description: `${accountToSuspend.name} has been ${action} successfully.`
      })
      setAccountToSuspend(null)
      setIsDialogOpen(false) // Close details dialog if open
    }
  }
  ...
  const confirmSuspendUser = () => {
    if (userToSuspend) {
      ...
      // TODO: API call to suspend/activate user
